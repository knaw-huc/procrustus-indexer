...
def make_json(cmdi):
    ...
    shelfmark = root.findall("./cmd:Components/cmd:eCodices/cmd:Source/cmd:Identifier/cmd:Idno/cmd:idno", ns)
    items = root.findall("./cmd:Components/cmd:eCodices/cmd:Source/cmd:Contents/cmd:Item", ns)
    if len(items) > 1:
        try:
            print(cmdi)
            print(shelfmark[0].text)
            print(len(items))
        except:
            pass
